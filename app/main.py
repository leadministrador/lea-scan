...
from .db import connect, init_db, DATA_DIR
...
@app.post("/upload")
async def upload(batch_name: str = Form(""), files: List[UploadFile] = File(...)):
        ...
        with connect() as conn:
            conn.execute("""
                INSERT INTO invoices (
                    batch_name, original_filename, stored_filename, sha256,
                    mime_type, file_size, supplier, supplier_tax_id,
                    invoice_number, invoice_date, currency,
                    net_amount, tax_amount, total_amount
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                batch_name.strip(), original_name, stored_name, digest,
                upload_file.content_type or "", len(data),
                extracted.get("supplier", ""), extracted.get("supplier_tax_id", ""),
                extracted.get("invoice_number", ""), extracted.get("invoice_date", ""),
...
def invoice_detail(request: Request, invoice_id: int):
    ...
    return templates.TemplateResponse(
        "invoice.html",
        {"request": request, "invoice": invoice, "items": items},
    )
...
def download_original(invoice_id: int):
    with connect() as conn:
        invoice = conn.execute(
            "SELECT original_filename, stored_filename FROM invoices WHERE id=?",
            (invoice_id,),
        ).fetchone()
    ...
    return FileResponse(str(path), filename=invoice["original_filename"], media_type="application/octet-stream")
